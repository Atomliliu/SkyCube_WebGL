...
function SC_OutHUD( cubeMap, width, height ) {
	...
	this.cameraHUD = new OrthographicCamera(-UI_width/2, UI_width/2, UI_height/2, -UI_height/2, 0, iconSize+50 );
	...
	function updateIconPlaneSize(){
		if(UI_IconPlane.length == UI_IconPos.length) {
			for ( var i = 0; i < UI_IconPlane.length; i ++ ) {
				...
				UI_IconPlane[i].position.copy( new Vector3(UI_IconPos[i].x, UI_IconPos[i].y, 0));
				
				if(i==4){
					UI_IconPlane[i].scale.copy( new Vector3(iconSize*UI_IconsSizeX[i], iconSize*UI_IconsSizeY[i],iconSize*UI_IconsSizeX[i]) );
					...
					UI_IconPlane[i].scale.copy( new Vector3(iconSize*UI_IconsSizeX[i], iconSize*UI_IconsSizeY[i],1) );
				}
